...
const METRICS = [
  { id:"receivable", label:"Total Receivable",  value:"$0.00", sub:"Across all open invoices",   icon:TrendingUp, color:"#6366f1", glow:"rgba(99,102,241,0.3)",  border:"rgba(99,102,241,0.25)",  bg:"rgba(99,102,241,0.08)" },
  { id:"utxos",      label:"Unclaimed UTXOs",   value:"0",     sub:"Pending in publicReceived",  icon:Inbox,      color:"#22d3ee", glow:"rgba(34,211,238,0.25)",  border:"rgba(34,211,238,0.22)",  bg:"rgba(34,211,238,0.07)" },
  { id:"claimed",    label:"Claimed Private",   value:"$0.00", sub:"In encrypted token account", icon:Shield,     color:"#10b981", glow:"rgba(16,185,129,0.25)",  border:"rgba(16,185,129,0.22)",  bg:"rgba(16,185,129,0.07)" },
  { id:"invoices",   label:"Invoices Issued",   value:"0",     sub:"All time",                   icon:FileText,   color:"#818cf8", glow:"rgba(129,140,248,0.25)", border:"rgba(129,140,248,0.22)", bg:"rgba(129,140,248,0.07)" },
];
  ...
  return (
    <div className="flex flex-col gap-7 animate-fade-in">
      ...
      <div className="flex items-center gap-4 rounded-2xl px-5 py-4"
        style={{ background:"linear-gradient(135deg,rgba(99,102,241,0.12) 0%,rgba(34,211,238,0.05) 100%)", border:"1px solid rgba(99,102,241,0.25)", boxShadow:"0 0 32px rgba(99,102,241,0.1)" }}>
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl"
          style={{ background:"rgba(99,102,241,0.2)", border:"1px solid rgba(99,102,241,0.3)" }}>
          <Shield className="h-5 w-5" style={{ color:"#818cf8" }} />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-body font-semibold" style={{ color:"rgb(248,250,252)" }}>Umbra registration required</p>
          <p className="text-body-sm" style={{ color:"rgb(148,163,184)" }}>
            Complete on-chain registration to start receiving private payments.
          </p>
        </div>
        <Link href="/settings">
          <button id="dashboard-register-btn"
            className="hover-lift shrink-0 flex items-center gap-1.5 rounded-xl px-4 py-2 text-sm font-semibold"
        ...
        <div className="lg:col-span-2 rounded-2xl overflow-hidden"
          ...
          <ul className="divide-y" style={{ borderColor:"rgba(255,255,255,0.04)" }}>
            {ACTIVITY.map(({ label, time, dot }) => (
              <li key={label} className="flex items-center gap-3.5 px-5 py-3.5 transition-colors hover:bg-white/[0.02]">
